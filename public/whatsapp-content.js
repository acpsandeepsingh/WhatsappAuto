...
function scoreSingleSearchText(query, normalizedQuery, normalizedQueryLower, relaxedQuery, rowText) {
  const text = cleanText(rowText);
  if (!text) return -1;
  const textLower = text.toLowerCase();
  const relaxedText = normalizeLooseText(text);
  const textPhone = normalizePhone(text);
  let score = -1;

  if (textLower === normalizedQueryLower) score = Math.max(score, 160);
  if (relaxedQuery && relaxedText === relaxedQuery) score = Math.max(score, 155);
  if (isPhoneMatch(query, text) || isPhoneMatch(normalizedQuery, text)) score = Math.max(score, 145);
  if (isLikelyPhoneQuery(query) && textPhone && textPhone.endsWith(normalizedQuery)) score = Math.max(score, 150);
  if (textLower.startsWith(`${normalizedQueryLower} `) || textLower.endsWith(` ${normalizedQueryLower}`)) score = Math.max(score, 125);
  if (textLower.includes(normalizedQueryLower) || normalizedQueryLower.includes(textLower)) score = Math.max(score, 100);
  if (relaxedQuery && relaxedText && (relaxedText.includes(relaxedQuery) || relaxedQuery.includes(relaxedText))) score = Math.max(score, 80);
  if (normalizedQuery && textPhone && (textPhone.includes(normalizedQuery) || normalizedQuery.includes(textPhone))) score = Math.max(score, 70);

  return score;
}

function scoreChatSearchMatch(query, normalizedQuery, normalizedQueryLower, relaxedQuery, rowText) {
  return scoreSingleSearchText(query, normalizedQuery, normalizedQueryLower, relaxedQuery, rowText);
}
...
async function typeSearch(query) {
  ...
  // âœ… FIX: Clear previous search before typing new one
  clearEditableBox(searchBox);
  await wait(150); // let WhatsApp react to the clear
...
function getPrimaryChatTitleRoot(cell) {
  if (!(cell instanceof HTMLElement)) return null;
  return (
    cell.querySelector('[role="gridcell"][aria-colindex="2"]') ||
    cell.querySelector('._ak8l') ||
    cell.querySelector('._ak8o') ||
    cell
  );
}

function getSidebarSectionLabel(cell) {
  if (!(cell instanceof HTMLElement)) return '';
  ...
  while (row?.previousElementSibling) {
    ...
    if (!text) continue;
    if (/^(chats|contacts|messages|groups)$/i.test(text)) return text;
  }
  return '';
}

function getChatRowTitleCandidates(cell) {
  if (!(cell instanceof HTMLElement)) return [];
  ...
  const preferredRoot = getPrimaryChatTitleRoot(cell) || cell;
  ...
  [
    ...
    preferredRoot.querySelectorAll(selector).forEach((node) => {
      if (!(node instanceof HTMLElement)) return;
  ...
  ['[data-testid*="cell-frame-title"]', '[data-testid*="chat-list-item-title"]', '[data-testid*="conversation-title"]'].forEach((selector) => {
    cell.querySelectorAll(selector).forEach((node) => {
      if (!(node instanceof HTMLElement)) return;
      pushCandidate(node.getAttribute('title'));
      pushCandidate(node.textContent);
    });
  ...
  pushCandidate(extractLikelyChatTitleFromRowText(getChatRowSearchText(cell)));

  return candidates.filter((text) => {
    if (!hasMeaningfulChatRowText(text)) return false;
    const lowered = text.toLowerCase();
    return !['download whatsapp', 'get extra features', 'sponsored', 'advertisement'].some((snippet) => lowered.includes(snippet));
  });
}

function getBestChatSearchMatchDetails(cell, query, normalizedQuery, normalizedQueryLower, relaxedQuery) {
  const titleCandidates = getChatRowTitleCandidates(cell);
  const fullRowText = getChatRowSearchText(cell);
  const sectionLabel = getSidebarSectionLabel(cell);
  const rect = cell.getBoundingClientRect();
  const candidates = [
    ...titleCandidates.map((text) => ({ text, source: 'title' })),
    { text: fullRowText, source: 'row' }
  ];

  const scored = candidates
    .map((candidate) => ({
      ...candidate,
      score: scoreSingleSearchText(query, normalizedQuery, normalizedQueryLower, relaxedQuery, candidate.text)
    }))
  ...
  return {
    ...
    rowTitle: titleCandidates[0] || extractLikelyChatTitleFromRowText(fullRowText),
    rowText: fullRowText,
    sectionLabel,
    visualTop: Math.round(rect.top || 0)
  };
}
...
function isStrongPhoneRowMatch(queryValue, rowTitle, rowText) {
  const queryPhone = normalizePhone(queryValue);
  if (queryPhone.length < 7) return false;

  const titlePhone = normalizePhone(rowTitle);
  const rowPhone = normalizePhone(rowText);
  if (titlePhone && (titlePhone.includes(queryPhone) || queryPhone.includes(titlePhone) || isPhoneMatch(queryPhone, titlePhone))) return true;
  if (rowPhone && (rowPhone.includes(queryPhone) || queryPhone.includes(rowPhone) || isPhoneMatch(queryPhone, rowPhone))) return true;
  return false;
}
...
function getSidebarSearchCandidates() {
  ...
  return normalized.filter((cell) => {
    ...
    const looksClickable =
      cell.matches?.('[role="gridcell"][tabindex]') ||
      Boolean(cell.querySelector('[role="gridcell"][aria-colindex="2"]')) ||
      Boolean(cell.querySelector('[role="button"]')) ||
      Boolean(cell.querySelector('span[dir="auto"][title]'));
...
async function openChat(queryValue) {
  ...
  const normalizedQueryText = normalizePhone(query);
  if (/^[+\d()\s-]+$/.test(query) && normalizedQueryText && normalizedQueryText.length < 7) {
    throw new Error(`Search query "${query}" is an incomplete phone number. Refusing to open a partial numeric match.`);
  }
  ...
  for (const variant of queryVariants) {
	...
	const variantNormalizedLower = variantNormalized.toLowerCase(); // âœ… correct for scoring
	...
	const scoredMatches = sidebarCells
	  .map((cell) => getBestChatSearchMatchDetails(
		cell, variant.value, variantNormalized, 
		variantNormalizedLower,  // âœ… was variantLower
		variantRelaxed
	  ))
    const filteredMatches = isLikelyPhoneQuery(variant.value)
      ? scoredMatches.filter((entry) => isStrongPhoneRowMatch(variant.value, entry.rowTitle, entry.rowText))
      : scoredMatches;
    const sortedMatches = filteredMatches.sort((a, b) => {
      const sectionPriority = (value) => {
        const normalized = cleanText(value).toLowerCase();
        if (normalized === 'contacts') return 0;
        if (normalized === 'chats') return 1;
        return 2;
      };
      return sectionPriority(a.sectionLabel) - sectionPriority(b.sectionLabel) || b.score - a.score || a.rowText.length - b.rowText.length;
    });
    const bestMatch = sortedMatches[0] || null;
    const secondBestMatch = sortedMatches[1] || null;
    const isSingleVisibleResult = filteredMatches.length === 1;
    const hasStrongPhoneMatch = isLikelyPhoneQuery(variant.value) && bestMatch
      ? isStrongPhoneRowMatch(variant.value, bestMatch.rowTitle, bestMatch.rowText)
      : false;
    const hasStrongMatch = hasStrongPhoneMatch || (bestMatch?.score || -1) >= 125;
    const hasAcceptableSingleResult = isSingleVisibleResult && (bestMatch?.score || -1) >= 80;
	const isAmbiguous =
	  Boolean(bestMatch && secondBestMatch) &&
	  bestMatch.score < 145 &&
	  bestMatch.sectionLabel === secondBestMatch.sectionLabel &&  // âœ… same section only
	  Math.abs(bestMatch.score - secondBestMatch.score) <= 10;
    const matchedCell = !isAmbiguous && (hasStrongMatch || hasAcceptableSingleResult) ? bestMatch?.cell || null : null;

    if (!matchedCell) {
      log('[Search][Results][FallbackSkipped]', {
        query,
        variant,
        reason: isAmbiguous ? 'ambiguous_top_results' : 'strict_match_required'
      });
      log('[Search][Results][NoMatch]', {
        ...
        queryLower: variantLower,
        relaxedQuery: variantRelaxed,
        candidateScores: scoredMatches.map((entry) => ({
          sectionLabel: entry.sectionLabel,
          rowTitle: entry.rowTitle,
    ...
    const matchedRowText = bestMatch?.rowText || getChatRowSearchText(matchedCell);
    const matchedRowTitle = bestMatch?.rowTitle || extractLikelyChatTitleFromRowText(matchedRowText);
    ...
    let switched = await confirmChatSwitched(variant.value, 3200, { clickedRowText: matchedRowText, clickedRowTitle: matchedRowTitle });
    switched = switched || (await confirmChatSwitched(variant.value, 9500, { clickedRowText: matchedRowText, clickedRowTitle: matchedRowTitle }));
    if (switched) {
...
async function confirmChatSwitched(expectedContact, timeoutMs = 12000, context = {}) {
  ...
  const clickedRowTitle = cleanText(context.clickedRowTitle || extractLikelyChatTitleFromRowText(context.clickedRowText || '')).toLowerCase();
  const expectedRelaxed = normalizeLooseText(expectedContact);
  const clickedRelaxed = normalizeLooseText(clickedRowTitle);
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    ...
    const titleRelaxed = normalizeLooseText(rawTitle);
	 if (
	  title &&
	  ((title === expected || 
		title.includes(expected) ||           // âœ… header contains query
		expected.includes(title) ||           // âœ… query contains header (truncated names)
		(expectedRelaxed && titleRelaxed === expectedRelaxed)) ||
		(expectedPhone && titlePhone && isPhoneMatch(expectedPhone, titlePhone)))
	) {
    ...
    if (
      clickedRowTitle &&
      title &&
      (title === clickedRowTitle ||
        (clickedRelaxed && titleRelaxed === clickedRelaxed) ||
        title.includes(`${clickedRowTitle} `) ||
        title.endsWith(` ${clickedRowTitle}`))
    ) {
